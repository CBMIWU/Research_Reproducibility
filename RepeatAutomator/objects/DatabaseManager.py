...
class DatabaseManager(object):
    ...
    def record_error(self, article_id='', identifier='', record_id='', method='', object_caller='', field='', value='', notes='', time=strftime("%Y-%m-%d %H:%M:%S", localtime())):
        ...
        c = conn.cursor()
        c.execute(
            "CREATE TABLE IF NOT EXISTS errortable(article_id TEXT, identifier TEXT, record_id TEXT, method TEXT, object TEXT, field TEXT, value TEXT, notes TEXT, datetimestamp TEXT)")
        ...
        try:
            c.execute("INSERT INTO errortable VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)",
                      (article_id, identifier, record_id, method, object_caller, field, value, notes, time))
            conn.commit()
            c.close()
            conn.close()
            return 0  # no errors
            ...
            try:
                c.execute("INSERT INTO errortable VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)", (str(article_id), str(identifier), str(
                    record_id), str(method), str(object_caller), str(field), str(value), str(notes), str(time)))
                conn.commit()
                c.close()
                conn.close()
                return 0  # no errors
            except Exception:
                sqlfields = ['article_id', 'identifier', 'record_id',
                             'method', 'object_caller', 'field', 'value', 'notes', 'time']
                error_index = re.search(r'(\d)', str(e)).group(1)
                self.record_error(method='record_error', object_caller='DatabaseManager',
                                  notes="invalid entry for field: '{}'".format(sqlfields[int(error_index)]))
                return 1  # errors occurred and couldnt be resolved
        except Exception as e:
            self.record_error(method='record_error', object_caller='DatabaseManager',
                              notes="error information from sqlite3: '{}'".format(e))
    ...
    def get_data(self, field):
        ...
        ch = pycurl.Curl()
        ch.setopt(
            ch.URL, 'https://redcap.wustl.edu/redcap/srvrs/prod_v3_1_0_001/redcap/api/')
        print(data)
            ...
            for eachdict in self.get_data(redcap):
                if (boolean):
                    if (eachdict[redcap].strip() == str(val)):
                        matches.append(
                            (eachdict['article_doi'], eachdict['record_id'], eachdict[redcap]))
                else:
                    if (eachdict[redcap].strip() != str(val)):
                        matches.append(
                            (eachdict['article_doi'], eachdict['record_id'], eachdict[redcap]))
        except KeyError as e:
            print("redcap field: '{}'\nnot found. did you mean: '{}'?\nverify and try again".format(
                redcap, get_close_matches(redcap, [d['field_name'] for d in self.get_metadata()])))
        return matches
        ...
        for item in self.get_metadata():
            if (item['field_name'] == redcap):
                if (item['field_type'] != "yesno"):
                    print("get_data called on invalid redcap field: {}\nget_ml_data can only be called on fields of field_type yesno but field type of {} is: '{}'".format(
                        redcap, redcap, item['field_type']))
                    return
        try:
            for eachdict in self.get_data(redcap):
                #mldata[eachdict['record_id'].strip()] = 1 if (eachdict[redcap].strip()=='1') else 0
                mldata[eachdict['record_id'].strip()] = eachdict[
                    redcap].strip() == '1'
        except KeyError as e:
